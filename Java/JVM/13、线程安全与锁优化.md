...
批量重偏向指同时将多个本该撤销为偏向锁重新偏向。

偏向锁可以提高带有同步但无竞争的程序性能，譬如当同一个线程反复进入同步代码的时候，偏向锁带的性能开销基本可以忽略。但当存在其他线程竞争时，偏向锁就需要等到安全点时撤销为无锁状态或升级为轻量级锁。相对而言，偏向锁的撤销会带来比较大的性能开销，在多线程竞争的情况下，偏向锁不仅不能提高性能，反而会导致性能下降。但好在偏向锁一旦撤销之后，就不会再偏向，所以这些开销也没有太大影响。但如果一段代码中有大量的偏向锁对象需要撤销呢？

例如，线程1创建大量Class A的实例对象的集合M，并对每一个实例对象进行同步加锁（偏向状态），待线程1结束之后，线程2对集合M中的每一个实例对象也进行同步加锁（偏向撤销：升级为轻量级锁），将会产生大量的撤销操作。

针对这种情况，HotSpot增加批量重偏向机制。

**批量重偏向机制：**

​		如果对同一个Class对象的大量实例锁对象执行撤销操作，撤销的锁对象的数量超过指定阈值，则不再进行撤销，后续的锁对象直接通过CAS将Mark Word中的JavaThread*替换为当前线程的指针.。

**批量重偏向原理：**

​		偏向锁撤销计数器：以Class为单位，每一个Class都维护了一个偏向锁撤销计数器，每当有一个该Class实例的锁对象发生偏向撤销操作时，其计数器就+1，当这个值达到重偏向阈值（默认20）时，就开始进行批量重偏向。

​		Epoch：每一个Class对象都有一个epoch字段，同时每个处于偏向锁状态的Mark Word也有一个用于记录epoch的bit位，其值等于该锁对象创建时其对应的Class对象中epoch字段的值。每次发生批量重偏向时，就将该Class对象中epoch的值+1，同时遍历JVM中所有线程的栈，找到该Class所有正处于加锁状态的偏向锁，将其epoch位设为新值，而对于没有处于加锁状态的偏向锁，其epoch仍为旧值。当epoch仍为旧值的锁对象下次再次被加锁时，发现其epoch与Class对象的epoch不相等，
