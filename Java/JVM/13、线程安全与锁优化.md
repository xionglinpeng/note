...
偏向锁的实现思路还是跟轻量级锁有一点相似之处。首先还是一样的，当一个线程进入同步代码块的时候，为锁对象加一个标记，标记当前是偏向状态，一直运行到同步代码块退出，然后另一个线程又执行同一个锁对象的同步代码块，获取偏向状态，一直重复这个过程。如果在同步代码还没有退出的时候，有另一个线程来竞争了，那么就将原偏向线程的同步对象升级为轻量级锁，然后以轻量级锁的方式执行。注意，一旦升级为轻量级锁之后，就永远不会在有偏向状态了，也就是说，之后即使没有竞争，也是以轻量级锁的方式执行。
...
在偏向锁执行未退出时，一旦有另一个线程尝试获取该锁，偏向模式就马上宣布结束。那么问题在于偏向模式是如何结束的呢？又如何升级为轻量级锁呢？考虑一个问题：当有另一个线程尝试获取该锁的时候，因为偏向线程的同步代码块还未退出，那么另一个线程必然要等待偏向线程的偏向模式结束，并升级为轻量级锁之后才能开始继续运行，然后便按照轻量级锁的模式运行。考虑一下偏向线程该怎么运行？

> 偏向模式标志位只有一位，即只能为0或1，0表示不可偏向，1表示可偏向。新建对象默认为1。
