...
3. "树的根要么是一片树叶"是指整棵B树只有根一个节点，意味着它既是根，也是树叶；"要么其儿子数在2~M之间"是指如果树的根不是树叶，有儿子节点，那么其儿子数必然在2~M之间，即最少有2个儿子，最多有M个儿子。进而可以推断其最少有1个关键，最多有M-1个关键字（两棵子树指针夹着一个关键字）。
